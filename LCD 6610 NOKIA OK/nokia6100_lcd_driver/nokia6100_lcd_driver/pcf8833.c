...
// File: pcf8833.c
//
// Display driver for the Nokia 6100 Display using the NXP PCF8833 controller.
// This implementation uses 12 bits per pixel.
// Large parts are based on the driver written by James P Lynch.
//
...
// *********************************************************************************
#if N6100_LCD_USE_FONT_SMALL == 1 && N6100_LCD_USE_FONTS == 1
...
#if N6100_LCD_USE_FONT_MEDIUM == 1 && N6100_LCD_USE_FONTS == 1
...
#if N6100_LCD_USE_FONT_LARGE == 1 && N6100_LCD_USE_FONTS == 1
...
// n6100_lcd_init()
//
// Initialize the PCF8833 LCD controller
//
// inputs: nothing
//
// Returns: nothing
...
void n6100_lcd_init()
{
	...
	//Sleep Out
	n6100_lcd_write_cmd(SLEEPOUT);

	// Display on
	n6100_lcd_write_cmd(DISPON);
	
	// Booster on
	n6100_lcd_write_cmd(BSTRON);
	
	//Normal display mode
	n6100_lcd_write_cmd(NORON);

	#if N6100_LCD_INVERT_RGB == 0
	//Display inversion off
	n6100_lcd_write_cmd(INVOFF);
	#elif N6100_LCD_INVERT_RGB == 1
	//Display inversion off
	n6100_lcd_write_cmd(INVON);
	#endif
	
	//Data order
	n6100_lcd_write_cmd(DOR);

	//Memory data access control
	n6100_lcd_write_cmd(MADCTL);

	// parse MADCTL options
	temp = 0x00;
	
	// see INVON/INVOFF
	//#if N6100_LCD_INVERT_RGB == 1
	//temp |= RGB_BIT;
	//#endif
	#if N6100_LCD_LINE_BOTTOP == 1
	temp |= LINE_ADDR_ORDER_BIT;
	#endif
	#if N6100_LCD_RAM_WR_VER == 1
	temp |= RAM_WR_V_BIT;
	#endif
	#if N6100_LCD_MIRROR_X == 1
	temp |= MIRROR_X_BIT;
	#endif
	#if N6100_LCD_MIRROR_Y == 1
	temp |= MIRROR_Y_BIT;
	#endif

	n6100_lcd_write_data(temp);

	// interface pixel format
	n6100_lcd_write_cmd(COLMOD);
	n6100_lcd_write_data(PIXEL_12BIT);   //12-Bit per Pixel (default)

	//Set Constrast
	n6100_lcd_write_cmd(SETCON);
	n6100_lcd_write_data(MAXCONTRAST);
...
void n6100_lcd_setxy(int x, int y)
{
	...
	n6100_lcd_write_cmd(PASET);
	n6100_lcd_write_data(x);
	n6100_lcd_write_data(x);
	
	// Column address set (command 0x2A)
	n6100_lcd_write_cmd(CASET);
	n6100_lcd_write_data(y);
	n6100_lcd_write_data(y);
}
#endif
...
// Returns: nothing
//
// Note: see nokia6100.h for some sample color settings
//
// Author: James P Lynch July 7, 2007
// *************************************************************************************
...
{
	n6100_lcd_setxy(x, y);
	n6100_lcd_write_cmd(RAMWR);
	n6100_lcd_write_data((unsigned char)((color >> 4) & 0xFFFF));
	n6100_lcd_write_data((unsigned char)(((color & 0x0F) << 4) | 0x00));
	n6100_lcd_write_cmd(NOP);
}
#endif
...
// see nokia6100.h for some sample color settings
//
...
void n6100_lcd_setline(int x0, int y0, int x1, int y1, int color)
{
	...
	n6100_lcd_setpixel(x0, y0, color);
...
//
// 					n6100_lcd_write_cmd(PASET); // set the row boundaries
// 					n6100_lcd_write_data(xmin);
// 					n6100_lcd_write_data(xmax);
// 					n6100_lcd_write_cmd(CASET); // set the column boundaries
// 					n6100_lcd_write_data(ymin);
// 					n6100_lcd_write_data(ymax);
//
...
// 						n6100_lcd_write_data((color >> 4) & 0xFF);
// 						n6100_lcd_write_data(((color & 0xF) << 4) | ((color >> 8) & 0xF));
// 						n6100_lcd_write_data(color & 0xFF);
// 					}
//
...
void n6100_lcd_setrect(int x0, int y0, int x1, int y1, unsigned char fill, int color)
{
	...
	if (fill == FILL) {
		...
		n6100_lcd_setline(x0, y0, x1, y0, color);
		n6100_lcd_setline(x0, y1, x1, y1, color);
		n6100_lcd_setline(x0, y0, x0, y1, color);
		n6100_lcd_setline(x1, y0, x1, y1, color);
	}
}
#endif
...
void n6100_lcd_setcircle(int x0, int y0, int radius, int color)
{
	...
	n6100_lcd_setpixel(x0, y0 + radius, color);
	n6100_lcd_setpixel(x0, y0 - radius, color);
	n6100_lcd_setpixel(x0 + radius, y0, color);
	n6100_lcd_setpixel(x0 - radius, y0, color);
	...
	while (x < y) {
		...
		ddF_x += 2;
		f += ddF_x + 1;
		n6100_lcd_setpixel(x0 + x, y0 + y, color);
		n6100_lcd_setpixel(x0 - x, y0 + y, color);
		n6100_lcd_setpixel(x0 + x, y0 - y, color);
		n6100_lcd_setpixel(x0 - x, y0 - y, color);
		n6100_lcd_setpixel(x0 + y, y0 + x, color);
		n6100_lcd_setpixel(x0 - y, y0 + x, color);
		n6100_lcd_setpixel(x0 + y, y0 - x, color);
		n6100_lcd_setpixel(x0 - y, y0 - x, color);
		}
...
//
// 		n6100_lcd_write_cmd(PASET); // set the row drawing limits
// 		n6100_lcd_write_data(20); //
// 		n6100_lcd_write_data(27); // limit rows to (20, 27)
//
// 		n6100_lcd_write_cmd(CASET); // set the column drawing limits
// 		n6100_lcd_write_data(20); //
// 		n6100_lcd_write_data(27); // limit columns to (20,27)
//
...
//
// 		n6100_lcd_write_cmd(RAMWR); // start a memory write (96 data bytes to follow)
//
// 		n6100_lcd_write_data(RRRRGGGG); // first pixel, red and green data
// 		n6100_lcd_write_data(BBBBRRRR); // first pixel, blue data; second pixel, red data
// 		n6100_lcd_write_data(GGGGBBBB); // second pixel, green and blue data
// 				:
// and so on until all pixels displayed!
// 				:
// 		n6100_lcd_write_cmd(NOP); // this will terminate the RAMWR command
//
//
// Author: James P Lynch July 7, 2007
// *****************************************************************************
...
void n6100_lcd_putchar(char c, int x, int y, int size, int fColor, int bColor)
{
	...
	unsigned char *FontTable[] = {
			#if N6100_LCD_USE_FONT_SMALL == 1
			(unsigned char *)FONT6x8,
			#else
			NULL,
			#endif
			
			#if N6100_LCD_USE_FONT_MEDIUM == 1
			(unsigned char *)FONT8x8,
			#else
			NULL,
			#endif
			
			#if N6100_LCD_USE_FONT_LARGE == 1
	...
	n6100_lcd_write_cmd(PASET);
	n6100_lcd_write_data(x);
	n6100_lcd_write_data(x + nRows - 1);
	
	// Column address set (command 0x2A)
	n6100_lcd_write_cmd(CASET);
	n6100_lcd_write_data(y);
	n6100_lcd_write_data(y + nCols - 1);
	...
	for (i = nRows - 1; i >= 0; i--) {
		...
		for (j = 0; j < nCols; j += 2) {
			...
			n6100_lcd_write_data((Word0 >> 4) & 0xFF);
			n6100_lcd_write_data(((Word0 & 0xF) << 4) | ((Word1 >> 8) & 0xF));
			n6100_lcd_write_data(Word1 & 0xFF);
		}
	}
...
void n6100_lcd_putstr(char *pString, int x, int y, int Size, int fColor, int bColor)
{
	...
	while (*pString != 0x00) {
		...
		// draw the character
		n6100_lcd_putchar(*pString++, x, y, Size, fColor, bColor);
